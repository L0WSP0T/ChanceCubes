...
public class ConfigLoader
{
	...
	//			.defineList("BlockedWorlds", new ArrayList<>(), input -> (input != null && !input.equals("")));
		...
		//CCubesSettings.spawnOnceEveryBlocks = builder
		//		.comment("Spawn a chance cube once every X blocks")
		//		.defineInRange("SpawnOnceEveryBlocks", 85, 0, 1000);
	...
	CCubesSettings.generateInNether = builder
			.comment("Generate chance cubes in the Nether?")
			.define("GenerateInNether", true);
		CCubesSettings.generateInEnd = builder
				.comment("Generate chance cubes in the End?")
				.define("GenerateInEnd", false);
